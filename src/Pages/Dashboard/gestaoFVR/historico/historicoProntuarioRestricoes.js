...
const HistoricoProntuarioRestricoes = ({ registros = [] }) => {
  ...
  const [dados, setDados] = useState([

    {
      id: 1,
      tipo: "Restrições",
      restricao: "Restrição de mobilidade devido a lesão no joelho.",
      recomendacoes: "Utilizar cadeira de rodas para locomoção. Evitar subir escadas.",
    ...
    <div>
      ...
      {dados.map((item) => {
        ...
        return (
          <Card
            key={item.id}
            className="shadow-sm border-start border-warning border-0 mb-3 rounded-3"
          >
            ...
            <Card.Body>
              ...
              <div className=" border-4 border-warning ps-3 mb-3">
                <strong className="text-warning">Recomendações</strong>
                <p className="mb-0 text-muted">
                  {item.recomendacoes || item.descricao || "-"}
                </p>
              </div>
            </Card.Body>
