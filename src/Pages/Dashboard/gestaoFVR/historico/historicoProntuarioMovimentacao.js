...
const HistoricoProntuarioMovimentacao = ({ registros = [] }) => {
  ...
  const [retornoMovimentacao, setRetornoMovimentacao] = useState("origem");
  const [novaAlocacaoMovimentacao, setNovaAlocacaoMovimentacao] = useState("");
  ...
  const handleAbrirModalConclusaoRegistro = (item) => {
    ...
    setRetornoMovimentacao("origem");
    setNovaAlocacaoMovimentacao("");
  ...
  const handleAbrirEdicao = (item) => {
    ...
    setEdicao({
      ...
      data_movimentacao: toInputDate(item.data_movimentacao),
    });
  ...
  const handleConcluirRegistro = async (e) => {
    ...
    if (retornoMovimentacao === "outra" && !novaAlocacaoMovimentacao) {
      setErroConclusaoRegistro("Selecione a alocação final para concluir a movimentação.");
      return;
    }
    ...
    try {
      const response = await api.concluirRegistro(registroSelecionado.id, senhaConclusaoRegistro, {
        movimentacao_retorno: retornoMovimentacao,
  ...
  const handleSalvarEdicao = async (e) => {
    ...
    try {
      const payload = {
        ...
        data_movimentacao: edicao.data_movimentacao || null,
      };
  ...
  return (
    <div>
      {dadosLocais.map((item) => {
        const statusAtual = item.movimentacao_status || item.status_conclusao || "";
        const isConcluido = String(statusAtual).toLowerCase() === "concluido";

        return (
          <Card key={item.id} className="shadow-sm border-start border-secondary border-0 mb-3 rounded-3">
            <Card.Header className="bg-white border-0 py-3 d-flex justify-content-between align-items-center">
              <div>
                <Badge bg="secondary" className="me-2 px-3 py-2">{item.tipo || "Movimentacao"}</Badge>
            ...
            <Card.Body>
              <div className="mb-3">
                <h6 className="text-secondary mb-1"><BsClipboardCheck className="me-1" />Origem</h6>
                <p className="mb-0 text-muted"><strong>Origem:</strong> {item.movimentacao_origem || item.origem || "-"}</p>
              </div>
               <div className="mb-3">
                <h6 className="text-secondary mb-1"><BsClipboardCheck className="me-1" />Destino</h6>
                <p className="mb-0 text-muted"><strong>Destino:</strong> {item.destino || "-"}</p>
              </div>
               <div className="mb-3">
                <h6 className="text-secondary mb-1"><BsClipboardCheck className="me-1" />Destino Final</h6>
                <p className="mb-0 text-muted"><strong>Destino Final:</strong> {item.movimentacao_destino_final || item.destino_final || "-"}</p>
              </div>

              <hr />

              <div className="border-4 border-secondary ps-3 mb-3">
                <strong className="text-secondary">Motivo</strong>
                <p className="mb-0 text-muted">{item.motivo || "-"}</p>
              </div>
            </Card.Body>
              ...
              <div className="border-4 border-warning ps-3 mb-3 w-100">
                <Row>
                  <Col xl={8}>
                    ...
                    <br />
                    <small className="text-muted"><strong>Última atualização:</strong> {item.movimentacao_usuario_atualizacao_nome || "-"} <strong>RE:</strong> {item.movimentacao_usuario_atualizacao_registro || "-"}</small>
                  </Col>
                  <Col xl={4}><p className="mb-0 text-muted">{item.movimentacao_data_atualizacao ? new Date(item.movimentacao_data_atualizacao).toLocaleDateString("pt-BR") : "-"}</p></Col>
                </Row>
              </div>
            </Card.Footer>
          </Card>
      ...
      <Modal show={showModalConclusaoRegistro} onHide={handleFecharModalConclusaoRegistro} centered>
        ...
        <Form onSubmit={handleConcluirRegistro}>
          <Modal.Body>
            <p className="text-muted mb-2"><strong>Origem da movimentação:</strong> {registroSelecionado?.movimentacao_origem || registroSelecionado?.origem || "-"}</p>
            <p className="text-muted mb-3"><strong>Destino atual:</strong> {registroSelecionado?.destino || "-"}</p>

            <Form.Group className="mb-3">
              <Form.Label className="fw-bold">Ao concluir, para onde o solípede deve ir?</Form.Label>
              <div>
                <Form.Check
                  type="radio"
                  id="retorno-origem"
                  name="retornoMovimentacao"
                  label="Retornar para a alocação de origem"
                  checked={retornoMovimentacao === "origem"}
                  onChange={() => setRetornoMovimentacao("origem")}
                />
                <Form.Check
                  type="radio"
                  id="retorno-outra"
                  name="retornoMovimentacao"
                  label="Finalizar em outra alocação"
                  checked={retornoMovimentacao === "outra"}
                  onChange={() => setRetornoMovimentacao("outra")}
                />
              </div>
            </Form.Group>

            {retornoMovimentacao === "outra" && (
              <Form.Group className="mb-3">
                <Form.Label>Alocação final</Form.Label>
                <Form.Select
                  value={novaAlocacaoMovimentacao}
                  onChange={(e) => setNovaAlocacaoMovimentacao(e.target.value)}
                  required
                >
                  {opcoesMovimentacao.map((opcao) => (
                    <option key={opcao} value={opcao}>
                      {opcao === "" ? "Selecione o destino final..." : opcao}
                    </option>
                  ))}
                </Form.Select>
              </Form.Group>
      ...
      <Modal show={showModalEdicao} onHide={handleFecharEdicao} centered size="lg">
        <Modal.Header closeButton><Modal.Title>Editar movimentacao</Modal.Title></Modal.Header>
        <Form onSubmit={handleSalvarEdicao}>
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              <Form.Control value={edicao.destino} onChange={(e) => setEdicao({ ...edicao, destino: e.target.value })} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label className="fw-bold">Data movimentacao</Form.Label>
              <Form.Control type="date" value={edicao.data_movimentacao} onChange={(e) => setEdicao({ ...edicao, data_movimentacao: e.target.value })} />
            </Form.Group>
          </Modal.Body>
          <Modal.Footer>
...
export default HistoricoProntuarioMovimentacao;
